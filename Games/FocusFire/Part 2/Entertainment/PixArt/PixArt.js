...
function checkGrid(){
	...
	if(!cmpArray(tempHintsH,hintsH) || !cmpArray(tempHintsV,hintsV)) return false;
...
function helpConfirm(){
	...
	for(i=0;i<hintsV.length;i++){
		...
		ttl = jiaks.length-2;
		for(j=0;j<jiaks.length-1;j++){
			ttl+=parseInt(jiaks[j]);
		}
		if(ttl==jiaks.length-2 || ttl==R){
			console.log("Found: "+hintsV[i]);
	...
	for(i=0;i<hintsH.length;i++){
		...
		if(ttl==jiaks.length-2 || ttl==C){
			console.log("Found: "+hintsH[i]);
